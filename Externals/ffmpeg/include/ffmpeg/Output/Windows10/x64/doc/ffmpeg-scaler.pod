...
=item B<srcw> I<(API only)>B<>

Set source width.


=item B<srch> I<(API only)>B<>

Set source height.


=item B<dstw> I<(API only)>B<>

Set destination width.


=item B<dsth> I<(API only)>B<>

Set destination height.


=item B<src_format> I<(API only)>B<>

Set source pixel format (must be expressed as an integer).


=item B<dst_format> I<(API only)>B<>

Set destination pixel format (must be expressed as an integer).


=item B<src_range> I<(boolean)>B<>

If value is set to C<1>, indicates source is full range. Default value is
C<0>, which indicates source is limited range.


=item B<dst_range> I<(boolean)>B<>
